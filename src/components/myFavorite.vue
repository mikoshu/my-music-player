...
<template>
	<div class="location">
		...
		<table class="list">
			...
			<tr v-if="!notFounded" v-for="(key,val) in favorite" >
				<td v-bind:class='[songId == key ? "playing" : ""]'>{{$index+1}}</td>
				<td class="w60" >
				...
				<td class="max-long-200">
					<a v-bind:class='[songId == key ? "weight" : ""]' data-id="{{val.songId}}" href="javascript:;" data-index="{{$index}}" v-on:click.stop="player" >{{{val.name}}}</a>
				</td>
